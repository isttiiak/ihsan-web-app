...
export const streakCoachHandler = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { event, streakDays, feature, bestStreak } = req.body as {
      event: 'milestone' | 'break'; streakDays?: number; feature: string; bestStreak?: number;
    };
    const result = await aiService.getStreakCoaching({ event, streakDays, feature, bestStreak });
    res.json({ ok: true, ...result });
  } catch (err) {
    next(err);
  }
...
export const fastingCompanionHandler = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { period, fastType, dayNumber } = req.body as {
      period: 'morning' | 'evening'; fastType: string; dayNumber?: number;
    };
    const result = await aiService.getFastingCompanion({ period, fastType, dayNumber });
    res.json({ ok: true, ...result });
  } catch (err) {
    next(err);
  }
...
export const activityInsightHandler = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { stats } = req.body as { stats?: Record<string, unknown> };
    const result = await aiService.getActivityInsight(stats ?? {});
    res.json({ ok: true, ...result });
  } catch (err) {
    next(err);
  }
