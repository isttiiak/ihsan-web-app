...
interface CachedWeekly { weekId: string; summary: string; encouragement: string }
interface CachedInsight { monthId: string; headline: string; insights: string[] }
...
export default function NaseehInsights() {
  ...
  const weeklyStats = useMemo(() => {
    ...
    return {
      zikrTotal7d: zikrData?.chartData?.reduce((a: number, d: { total?: number }) => a + (d.total ?? 0), 0) ?? 0,
  ...
  useEffect(() => {
    ...
    if (cached) { setWeekly(cached); return; }
    if (!weeklyStats || weeklyMut.isPending) return;
    weeklyMut.mutate(weeklyStats, {
      onSuccess: (r) => {
        const entry: CachedWeekly = { weekId: wk, summary: r.summary, encouragement: r.encouragement };
        setWeekly(entry);
