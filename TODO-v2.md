...
### 1.4 Auth Hardening ✅ (2026-09-06)

- [x] **Firebase account deletion** — `admin.auth().deleteUser(uid)` was already wired into `deleteAccount()`, but investigating turned up a real bug: it was called unconditionally, and threw synchronously ("app/no-app") in any environment without full Firebase service-account credentials (local dev without them, `DEV_AUTH_BYPASS`) — turning a fully successful Mongo purge into a 500, with the user never told their data was actually wiped. Fixed by guarding with `isFirebaseInitialized()` (same check `verifyFirebaseToken` already uses); production behavior (real Firebase Admin configured) is unchanged. New test reproduces the pre-fix 500 and confirms the post-fix 200 + full purge.
- [x] **Session timeout for sensitive ops** — scoped to account deletion specifically (the one truly irreversible, full-data-loss action): account _linking_ already forces fresh auth implicitly via `linkWithPopup`'s live Google consent screen, and unlinking is reversible/doesn't touch data, so neither needed more hardening. Frontend (`Settings.tsx`): the delete-account confirm step now requires re-authentication first — a password field + `reauthenticateWithCredential` for email/password users, or a "Re-verify with Google" button + `reauthenticateWithPopup` for Google-only users — before the delete call fires. Backend (`requireRecentAuth` in `middleware/auth.ts`): a stolen/replayed bearer token could otherwise call `DELETE /api/user/me` directly and skip the frontend prompt (a Firebase ID token stays valid for an hour regardless of when the user actually signed in) — rejects with `reauth_required` unless the token's `auth_time` claim is within the last 5 minutes. No-ops when `auth_time` is absent (dev-bypass tokens only; Firebase-verified tokens always carry it).
